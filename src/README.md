...
# ESM Evaluation Pipeline (`esm_eval/`)

A standalone module for evaluating protein sequence quality using ESM log-likelihood scores. This provides quantitative comparison of sequence sets.
...
**Outputs:**
- Console summary with KS statistics and Wasserstein distances
- CSV file with pairwise distance matrices

## ESM Evaluation Files

| File | Purpose |
|------|---------|
| `run_esm_eval.py` | Main CLI entry point |
| `esm_scoring.py` | ESM model scoring (log-likelihood computation) |
| `load_sequences.py` | Sequence loading and validation |
| `plot_histograms.py` | Histogram visualization |
| `compute_distances.py` | KS and Wasserstein distance computation |
